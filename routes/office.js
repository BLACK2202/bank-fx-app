...
function buildRows(operations) {
  return operations.map((op) => {
    const finalAmount =
      op.taux_final != null ? op.montant * op.taux_final : null;
    return {
      id: op.id,
...
function listOperationsForRole(role, startDate, endDate, searchTerm) {
  const operations = ops.listAllBetween(startDate, endDate, searchTerm);
  if (role !== "back_office") return operations;

  return operations.filter(
    (operation) =>
      operation.status === "validated_by_siege" &&
      String(operation.reference_number || "").trim() !== "",
  );
}

router.get("/activity", (req, res) => {
  if (req.session.user.role !== "back_office") {
    return res
      .status(403)
      .render("error", { message: "Acces reserve au Back Office." });
  }
  const searchTerm = req.query.search ? String(req.query.search).trim() : "";
  const activities = activity.listActivity({ searchTerm });
  res.render("office_activity", { activities, searchTerm });
});

router.get("/", (req, res) => {
  const { startDate, endDate } = resolvePeriod(req.query);
  const searchTerm = req.query.search ? String(req.query.search).trim() : "";
  const operations = listOperationsForRole(
  ...
  res.render("office_dashboard", {
    ...
    endDate,
    searchTerm,
    officeRole: req.session.user.role,
    error: null,
  });
});
...
router.get("/export", async (req, res, next) => {
  try {
    ...
    sheet.columns = [
      ...
      { header: "Reference siege", key: "reference_number", width: 20 },
    ...
    const fileName = `${req.session.user.role === "back_office" ? "operations_validees" : "operations"}_${startDate}_${endDate}.xlsx`;
    res.setHeader(
      "Content-Type",
